...
impl Topology {
    ...
    /// light_mk5: where a present device is attached -- `(hub address, 1-based port)`, the hub
    /// address being 0 for the root port -- or `None` if it is not present. A status display
    /// showing which socket an instrument is in wants exactly this.
    pub fn parent_of(&self, device: u8) -> Option<(u8, u8)> {
        match self.parent.get(device as usize) {
            Some(&entry) if entry > 0 => Some((entry & 15, entry >> 4)),
            _ => None,
        }
    }
