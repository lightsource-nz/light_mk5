...
impl UsbShared {
    ...
    pub fn on_irq(&self) {
        ...
        if (ints.bits() & 1) != 0 {
            ...
            // (light_mk5: this pac has no writer for SPEED, so the bits are named by position)
            unsafe { regs.sie_status().modify(|r, w| w.bits(r.bits() | (3 << 8))) };
...
/// Implementation of `HostController::DeviceDetect` for RP2350
#[derive(Copy, Clone)]
pub struct Rp2350DeviceDetect {
    waker: &'static CriticalSectionWakerRegistration,
    status: DeviceStatus,
}

impl Rp2350DeviceDetect {
...
impl Stream for Rp2350DeviceDetect {
...
struct Rp2350ControlEndpoint<'a> {
    waker: &'a CriticalSectionWakerRegistration,
}

impl<'a> Rp2350ControlEndpoint<'a> {
    fn new(waker: &'a CriticalSectionWakerRegistration) -> Self {
        Self { waker }
    }
}

impl Future for Rp2350ControlEndpoint<'_> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if (intr.bits() & 0x458) != 0 {
            ...
            // light_mk5: NOT cleared here. The flags were read a moment ago and found clear; a
            // completion that lands between that read and a write here is wiped before anyone
            // sees it, and the transfer never resolves -- nor does anything queued behind it.
            // Polled continuously rather than woken from the interrupt, this future hit that
            // window within a few enumerations. The flags are cleared once at the start of a
            // transfer (send_setup) and once when the completion is consumed above
            defmt::trace!(
...
/*
struct Rp2350BulkEndpoint<'a> {
    n: u8,
    waker: &'a CriticalSectionWakerRegistration,
}

impl<'a> Rp2350BulkEndpoint<'a> {
    pub fn new(n: u8, waker: &'a CriticalSectionWakerRegistration) -> Self {
        Self { n, waker }
    }
}

impl Future for Rp2350BulkEndpoint<'_> {
    type Output = ();
...
/// Implementation of `HostController::InterruptPipe` for RP2350
pub struct Rp2350InterruptPipe {
...
impl Stream for Rp2350InterruptPipe {
...
/// Implementation of HostController for RP2350
pub struct Rp2350HostController {
    shared: &'static UsbShared,
...
impl Rp2350HostController {
    /// Create a new RP2350HostController
    ///
    ...
    ///
    /// See the RP2040 module for a complete working example.
    pub fn new(
        ...
        regs.main_ctrl().modify(|_, w| {
            ...
            // light_mk5: the RP2350's PHY resets ISOLATED, and a modify keeps it so: a fully
            // configured controller that never sees the bus
            w.phy_iso().clear_bit();
        ...
        unsafe {
            // (light_mk5: this pac does not re-export NVIC; the core's is the same block)
    ...
    async fn send_setup(
        ...
        loop {
            let f = Rp2350ControlEndpoint::new(&self.shared.pipe_wakers[0]);
    ...
    async fn bulk_transfer_inner(
        ...
        loop {
            ...
            let f =
                Rp2350BulkEndpoint::new(pipe.n, &self.shared.pipe_wakers[n]);
            f.await;
...
impl HostController for Rp2350HostController {
    type InterruptPipe = Rp2350InterruptPipe;
    type DeviceDetect = Rp2350DeviceDetect;

    fn device_detect(&self) -> Self::DeviceDetect {
        Rp2350DeviceDetect::new(&self.shared.device_waker)
    }
